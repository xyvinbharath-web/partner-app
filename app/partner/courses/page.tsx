  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 pb-10">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Your Courses</h1>
      ...
      <section className="rounded-3xl border border-slate-100 bg-white/90 p-5 shadow-lg/40 backdrop-blur-sm md:p-6">
        <h2 className="text-sm font-semibold text-slate-900">Create new course</h2>
        ...
        <form onSubmit={handleCreateCourse} className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-1.5">
            ...
            <select
              name="category"
              className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500/60"
              defaultValue=""
            >
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              type="file"
              accept="image/*"
              className="block w-full cursor-pointer rounded-xl border border-dashed border-slate-300 bg-slate-50 px-3 py-2 text-xs text-slate-700 hover:border-emerald-400 hover:bg-emerald-50/40"
            />
          </div>
          <div className="space-y-1.5">
            <label className="block text-xs font-medium text-slate-700">
          ...
          <div className="md:col-span-2 flex justify-end">
            <button
              type="submit"
              className="inline-flex items-center rounded-full bg-slate-900 px-4 py-2 text-xs font-semibold tracking-wide text-white shadow-sm hover:bg-slate-800 hover:shadow-md transition-all"
      ...
      {!isLoading && courses && courses.length > 0 && (
        <section className="overflow-hidden rounded-3xl border border-slate-100 bg-white/95 shadow-xl">
          <div className="flex items-center justify-between border-b border-slate-100/80 bg-slate-50/70 px-4 py-3.5 md:px-5">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Courses overview</h2>
              <p className="text-[11px] text-slate-500">Summary of your published and draft courses.</p>
            </div>
            <span className="hidden rounded-full bg-white px-3 py-1.5 text-[11px] font-semibold text-slate-700 shadow-sm md:inline-flex">
            ...
            <table className="min-w-full border-separate border-spacing-0 text-sm">
              <thead>
                <tr className="bg-slate-50 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                  <th className="px-4 py-2 text-left md:px-5">Course</th>
                  <th className="px-4 py-2 text-left md:px-5">Price</th>
                  <th className="px-4 py-2 text-left md:px-5">Views</th>
                  <th className="px-4 py-2 text-left md:px-5">Earnings</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {courses.map((course, idx) => (
                  <tr
                    key={course._id}
                    className={`text-[13px] text-slate-700 transition-colors ${
                      idx % 2 === 0 ? "bg-white" : "bg-slate-50/60"
                    } hover:bg-slate-50/80`}
                  >
                    <td className="max-w-xs px-4 py-3.5 align-top md:px-5">
                      <div className="truncate text-[13px] font-semibold text-slate-900">{course.title}</div>
                      <div className="mt-0.5 text-[11px] text-slate-500">
                        {course.category || "Uncategorized"}
                      </div>
                    </td>
                    <td className="px-4 py-3.5 align-top text-[13px] text-slate-700 md:px-5">
                    ...
                    <td className="px-4 py-3.5 align-top text-[13px] text-slate-700 md:px-5">
                      <span className="inline-flex rounded-full bg-emerald-50 px-2.5 py-0.5 text-[11px] font-medium text-emerald-700">
                        {course.partnerEarnings != null ? `₹${course.partnerEarnings}` : "₹0"}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3.5 align-top md:px-5">
                      <div className="space-y-2 text-right">
                        <div className="flex justify-end gap-2">
                          <button
                            type="button"
                            className="inline-flex items-center rounded-full border border-emerald-500/40 bg-emerald-50 px-3 py-1 text-[11px] font-semibold text-emerald-700 hover:bg-emerald-100 hover:border-emerald-500 transition-colors"
                            onClick={() =>
                        ...
                        {expandedCourseId === course._id && (
                          <div className="mt-2 space-y-2 rounded-2xl bg-slate-50 p-3 text-xs text-left">
                            ...
                            >
                              ...
                              <div className="flex justify-end pt-1">
                                <button
                                  ...
                                  className="inline-flex items-center rounded-full bg-slate-900 px-3 py-1 text-[11px] font-semibold text-white shadow-sm hover:bg-slate-800 hover:shadow-md transition-all"
                                >
